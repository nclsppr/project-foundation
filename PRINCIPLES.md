...
## P21. Emit safe, structured, and actionable log records

**Rule.** If a project emits first-party runtime log records, each log record must be structured. A runtime log record is diagnostic or operational telemetry that first-party code sends to a logging sink or writes to a stream for collection as telemetry. A defined user-facing command result and build or test progress output is not a runtime log record. If a platform collects that output as application telemetry, the platform mapping applies `P21`. Calling diagnostic telemetry console output does not exempt it from this rule. A log record represents one occurrence. Its event name identifies the stable type of occurrence.

Each log record contains the occurrence time in UTC, a normalized severity that is one of `DEBUG`, `INFO`, `WARN`, or `ERROR`, a stable service or executable identifier, and a short English message from a constant call-site template. Add the deployed revision and environment when known. Add the active trace and span identifiers, or another trusted correlation identifier, when the occurrence belongs to an operation. Add a stable, low-cardinality error type when the occurrence is a failure. Every `INFO`, `WARN`, and `ERROR` log record has a stable, lowercase, dot-separated event name. A `DEBUG` log record needs an event name only when an automated or operational consumer depends on it. Put variable values in typed structured fields. Do not put them only in the message. Do not put high-cardinality values in event names or field names.

Use `DEBUG` for detailed diagnostic information that is not necessary during normal operation. Use `INFO` for an expected and significant lifecycle, business, or state transition. Use `WARN` for an unexpected degraded or at-risk condition when no technical operation has reached a failed final outcome. This level includes an operation that completes or recovers through a fallback. Use `ERROR` when a technical operation reaches a failed final outcome, data consistency is uncertain, or the process cannot continue. An expected input rejection, negative business result, routine client cancellation, cache miss, or individual retry attempt is not a technical error.

Normalize framework-specific levels to these four meanings before applying the contract. Treat `TRACE` as `DEBUG`. Treat `FATAL` as `ERROR` and add a structured process-termination outcome. Map any other level to its closest semantic meaning. Apply the event-name requirement after this mapping. Do not map numeric severity values directly between formats. A platform can preserve its native severity in a separate optional field.

Log an exception once, in the component that handles the failure or reports its final outcome. Preserve its cause. Do not log it and rethrow it unchanged. Pass the exception object to the logging interface only when the logging path applies the `P21` data controls before the log record reaches any sink. Otherwise, extract approved exception fields explicitly. Record the exception type. Record its stack trace when the trace is useful and the data-protection rules permit it.

Each message follows `P20` and describes one occurrence. Its template is constant at the call site, but its wording is not a compatibility contract. Use both the stable event name and the human-readable message. Do not introduce an opaque numeric log code unless an external contract requires it. Keep an external or user-facing error code separate from the internal event name. An automated consumer selects the stable event name and fields. It does not match message text.

Apply `P11` to every level. Do not record a secret, credential, token, session value, connection string, raw authorization data, or raw request or response body. Record personal or other sensitive data only when an approved purpose requires the minimum value. Treat exception messages as potentially sensitive. Sanitize and bound untrusted values before logging. Define access, retention, and deletion for retained log records.

Keep audit and security log records separate from diagnostic verbosity. When they apply, identify the actor or originating system, action, target reference, outcome, occurrence time, origin, and correlation. Apply the preceding data-minimization rule: record no secrets and only the approved minimum personal or sensitive data. The application log level cannot disable these records. Do not sample them. Define their access, integrity, retention, deletion, and failure behavior according to the applicable legal or security contract. That contract can require the protected action to stop when audit recording fails.

`INFO` and `DEBUG` log records can be sampled under a documented policy. Do not sample `WARN` and `ERROR` log records during normal operation. A volume-protection control can suppress repeated `WARN` or `ERROR` log records only if it preserves one representative log record and a separate bounded counter counts observed and suppressed occurrences before export. Document whether exporter failure can lose that counter. Diagnostic log export must not become an unbounded queue or a synchronous remote dependency, and its failure must not change a reported business result.

Log severity does not define alert priority. An alert uses stable fields and an actionable condition, owner, response, and runbook. Prefer user-visible symptoms and service objectives to generic counts of log records.

`P21` cannot be disabled for first-party runtime log records. A platform mapping can change serialized field names, enrichment location, presentation, or transport. It preserves the required semantics and documents the mapping. An external source or legacy format that cannot meet this rule requires an explicit and limited exception. The exception identifies the source, missing fields, compensating mapping or isolation, owner, and review or removal condition.

**Reason.** Free text alone is unstable for queries and alerts. A code alone does not explain the occurrence to an operator. Structured log records support correlation, automation, and diagnosis. Controlled messages support people. Correct severity, bounded volume, and data minimization reduce alert fatigue, operating cost, and disclosure risk.

**Minimum evidence.** The project verification command checks representative log records at each severity that the project uses for the required schema, severity, applicable correlation, and synthetic sensitive-data exclusion. When a component can report a failed technical operation, a failure-path test shows one `ERROR` log record from the component that reports the final outcome. Each automated consumer selects a stable event name or field, not message text. The project records its production threshold, sampling, retention, audit applicability, and alert ownership. A project that emits no first-party runtime log records marks this principle as not applicable.

## P22. Verify and adopt the latest stable Foundation release before each commit

**Rule.** Each project that adopts Project Foundation records the official source, stable release tag, complete commit, adopted pack, activated profiles, and SHA-256 hashes in `foundation.lock.json`. A stable release uses an annotated `vMAJOR.MINOR.PATCH` tag. Version precedence follows Semantic Versioning. The complete commit and file hashes define the adopted content. A moving branch does not define a release.

Before each commit, an online gate resolves the greatest stable release tag from the recorded source. The gate verifies that the adopted tag still identifies the recorded commit. It also verifies each vendored snapshot file and each Foundation-managed control against the lock. An unavailable source, an absent or moved tag, an obsolete release, a missing file, or a hash difference blocks the commit. A local exception cannot disable this gate or declare an unverified release current.

When a newer release exists, the gate prepares a controlled update. It replaces only the read-only snapshot and controls that the Foundation distribution manifest identifies. It does not overwrite a local exception, local project rule, application quality gate, or another local file. A collision, removal, or local modification of managed content stops the automatic update and requires a manual migration. The first commit attempt stops after it prepares an update. The contributor reviews the complete diff and Foundation changelog. The contributor then applies each changed rule, reconciles exceptions, updates local adapters and the project changelog, and runs the complete project verification before a new commit attempt.

The repository versions the pre-commit hook and the online CI check. Local Git configuration activates the hook. The canonical branch makes the CI check required because Git permits a contributor to bypass a local hook. A trusted mirror can replace the network location only when it preserves the official annotated tags and complete commits.

This principle applies to new projects and existing projects that adopted an earlier Foundation release. An existing project must perform one reviewed migration to the first release that contains `P22`. All later commits use the continuous check. The Project Foundation repository is the upstream source and does not adopt itself.

**Reason.** A vendored snapshot becomes obsolete unless each project compares it with the canonical release source. Silent drift leaves projects on old security, quality, and operating rules. A version, complete commit, and file hashes make the adopted content explicit. A local hook gives immediate feedback. An independent CI check prevents a hook bypass from reaching the canonical branch.

**Minimum evidence.** `foundation.lock.json` matches `FOUNDATION.md`, the vendored snapshot, and each Foundation-managed control. The local repository uses the versioned pre-commit hook. The hook runs the project verification command. CI resolves the release source again and checks the lock on each push, pull request, and merge-queue candidate. Tests cover a current release, an obsolete release, a moved tag, unavailable source, snapshot drift, managed-file collision, and a prepared update that preserves local files. The canonical branch requires the Foundation CI result.
